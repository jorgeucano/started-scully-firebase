...
asldofjhaskldfjhalksjdfhkljasdjhflkasdhjfkljh
## Table of contents
...
```
Toda la _magia_ de RxJS empieza en los **observables**.Pero Antes de ver los Observables vamos a entender porque quienes los usamoslos amamos tanto.Al principio de los tiempo :Si usabamos PHP (por ejemplo) teniamos que hacer un get a algo y el servidoresperaba a tener todas las respuestas paso a paso para poder trabajarLUEGO en el inicio de javascript teniamos que hacer las llamadas por medio de AJAX o XHR, lo cual hacia quepor cualquier error tuvieramos que manejarnos a lo picapiedraDespues aparecio **CALLBACK** una forma de dejar continuar el flujo y poderhacer 1 sola llamada, que ya nos daba cierta gloria manejar cosasasincronicamentePERO solo se podia hacer una llamada y todo lo demas lo teniamos que manejar a mano, como por ejemplo cuantas cosas dejabamos que haga el usuariomientras esperabamos ese **_CALLBACK_**<pre>var count = 0;var rate = 1000;var lastClick = Date.now() - rate;var button = document.querySelector('button');button.addEventListener('click', () => {  if (Date.now() - lastClick >= rate) {    console.log('Clicked', ++count, ' times');    lastClick = Date.now();  }});</pre>HASTA QUE aparecio Netflix y sus grandes ingenieros de magia![](assets/magic.gif)<pre>import { fromEvent } from 'rxjs/observable/fromEvent';import { scan, throttleTime } from 'rxjs/operators';var button = document.querySelector('button');fromEvent(button, 'click').pipe(  throttleTime(1000),  scan(count => count + 1, 0))  .subscribe(count => console.log('Clicked', ++count, ' times'));</pre>
